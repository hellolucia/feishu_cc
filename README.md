...
## 使用注意
1. 本项目初始启动时claude code运行在初始配置时指定的默认工作目录下，建议为每个项目创建单独的文件夹，每次项目运行时/project <文件夹名称>切换到对应项目，每次切换项目时会创建新session（用/projects命令可列出所有的项目文件夹）
2. 这里启动的CC在同一个项目下除非手动/new，否则都是同一个session，如果session满了，请自行/new或clear进行上下文管理
3. 这里运行的CC是（bypass permissions）的模式，也就是什么操作都可以直接执行，虽然在prompt里约定了，删除等操作还是要做二次确认，但仍然不建议执行此类危险操作
4. 为了以防万一，不建议把机器人加入群聊
